...
for i in range(total_photos):
    ...
    if orientation == 'H':
        horizontal_images_arr.append(new_object)
    else:
        vertical_images_arr.append(new_object)
...
while count < total_slides:
    for y in horizontal_images_arr:
        if not y.used:
            new_slide.slide_desc.append(horizontal_images_arr.index(y))
            y.used = True
            count += 1

    new_arr = []
    for y in vertical_images_arr:
        if not y.used and len(new_arr) < 2:
            new_arr.append(vertical_images_arr.index(y)+ len(horizontal_images_arr))
            y.used = True
